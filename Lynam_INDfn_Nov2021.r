...
INDfn <- function(DATA, WRITE=F, BOOT=F, LFI=T, LFI_THRESHOLD=NULL, FILENAM="", SAMP_STRAT=T, BYSUBDIV=T, AREASCALE=T,
  ...
  #DATA<-dhspp; GROUP<-NULL;
    ...
    ##          if(!XYmatch[[m]][3] %in%  DATA[DATA$Year==YR,"sampstrat"]) next #skip quads where no original sample within i.e. all from smooth
      ...
      # species recrods by rect (strata1) and subdivision (strata2) from haul data
      if(!BYSUBDIV & SAMP_STRAT) DATANOLEN<-aggregate(x=DATA$DensBiom_kg_Sqkm, by=list(Year=DATA$Year, SpeciesSciName=DATA$SpeciesSciName, Group=DATA$Group, HaulID=DATA$HaulID, sampstrat=DATA$sampstrat), FUN=sum)  
      if(BYSUBDIV & !SAMP_STRAT) DATANOLEN<-aggregate(x=DATA$DensBiom_kg_Sqkm, by=list(Year=DATA$Year, SpeciesSciName=DATA$SpeciesSciName, Group=DATA$Group, HaulID=DATA$HaulID, subdiv=DATA$subdiv), FUN=sum)  
      if(BYSUBDIV &  SAMP_STRAT) DATANOLEN<-aggregate(x=DATA$DensBiom_kg_Sqkm, by=list(Year=DATA$Year, SpeciesSciName=DATA$SpeciesSciName, Group=DATA$Group, HaulID=DATA$HaulID, sampstrat=DATA$sampstrat,subdiv=DATA$subdiv), FUN=sum)  
      
      if(SAMP_STRAT & !BYSUBDIV) species_rec_by_area <- aggregate(x=DATANOLEN$x, by=list(Year=DATANOLEN$Year, SpeciesSciName=DATANOLEN$SpeciesSciName, sampstrat=DATANOLEN$sampstrat), FUN=length)  
      if(BYSUBDIV & !SAMP_STRAT) species_rec_by_area <- aggregate(x=DATANOLEN$x, by=list(Year=DATANOLEN$Year, SpeciesSciName=DATANOLEN$SpeciesSciName, subdiv=DATANOLEN$subdiv), FUN=length)  
      if(BYSUBDIV &  SAMP_STRAT) species_rec_by_area <- aggregate(x=DATANOLEN$x, by=list(Year=DATANOLEN$Year, SpeciesSciName=DATANOLEN$SpeciesSciName, sampstrat=DATANOLEN$sampstrat, subdiv=DATANOLEN$subdiv), FUN=length)  
      names(species_rec_by_area)[length(names(species_rec_by_area))]<-"N"
      if(WRITE  & (!BOOTSTRAP | (BOOTSTRAP & B==0)  ) ) write.table(species_rec_by_area,paste(FILENAM,SPECIES,"species_records_BY_reg_yr.csv",sep="_"),row.names =F,sep=',')
      rm(DATANOLEN)
      
      species_rec_by_area<- aggregate(species_rec_by_area$N, by=list(Year=species_rec_by_area$Year, SpeciesSciName=species_rec_by_area$SpeciesSciName), FUN=sum)
      names(species_rec_by_area)[length(names(species_rec_by_area))]<-"N"
      if(WRITE  & (!BOOTSTRAP | (BOOTSTRAP & B==0)  ) ) write.table(species_rec_by_area,paste(FILENAM,SPECIES,"species_records_BY_yr.csv",sep="_"),row.names =F,sep=',')
      rm(species_rec_by_area)
