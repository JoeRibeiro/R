...
##second level raising if needed, here weighting is correct for coverage of SurvStratum (sum of sampstrat areas should be SurvStratum area - but prone to issues with missing sampstrat)
...
INDfn <- function(DATA, WRITE=F, BOOTSTRAP=F, LFI=T, LFI_THRESHOLD=NULL, FILENAM="", SAMP_STRAT=T, BYSUBDIV=T, AREASCALE=T,
  ...
  if(BYSUBDIV)   FACTHAUL <-  c(FACTHAUL,"SurvStratum","STRAT_DIV")
  ...
  if(QUAD){
    ...
    if(QUAD_SMOOTH){
      ...
      #add back SurvStratum STRAT_DIV centlon centlat fguild
    ...
    #SurvStratum area - if using by rectangle sampstrat need to sum area for SUBDIV
    if(survey %in% c("GNSIntOT1","GNSIntOT1_channel","GNSIntOT3","GNSNetBT3","GNSGerBT3","GNSBelBT3", "GNSNetBi3", "GNSIntBi3")){
      ...
      names(ATTRIB_SUBDIV) <- c("SurvStratum","KM2_LAM")
    }
    ...
    ox <- over(dhspp0, SUBDIV) #bring in all attributes of location i..e both sampstrat and SurvStratum if applicable 
    ...
    names(DATA)[ncol(DATA)]<-"SurvStratum"
    if(QUAD) DATA<- DATA[!is.na(DATA$SurvStratum),]#plot(sdr); map(add=T); with(DATA[is.na(DATA$SurvStratum),],points(cent_lon,cent_lat,pch=19,col=4)) # a couple of odd points - poss on land - can be smoothed in
  } #end quad 
  ##DATA is updated so have smooth total catch from hauls <60km from q_center by quadrant
  #DATA[DATA$SurvStratum!=DATA$SurvStratum,]
  ...
  if(SAMP_STRAT){ #might be STSQ, QUADrants or minigrid see sampstrat
    ...
    if(QUAD & QUAD_SMOOTH){ 
      ...
      numhaulsBYsampstrat <- aggregate( x=DATA$numhauls,by=list(Year=DATA$Year,sampstrat=DATA$sampstrat,SurvStratum=DATA$SurvStratum),FUN=mean)
    ...
    #and reshape since have one value per year and SurvStratum combination
  ...
  if(BYSUBDIV){#user_defined or survey poly
    if(!QUAD) numhaulsBYsubdiv <- tapply.ID(df=numhauls, datacols=c("ones"),
                                            factorcols=c("Year","SurvStratum"), sum,c("numhauls"));
    if(QUAD)  numhaulsBYsubdiv <- tapply.ID(df=numhaulsBYsampstrat, datacols=c("numhauls"),
                                            factorcols=c("Year","SurvStratum"),sum,c("numhauls"));
    #and reshape since have one value per year and SurvStratum combination
    numhaulsBYsubdivout <- (tapply(numhaulsBYsubdiv$numhauls,list(numhaulsBYsubdiv$Year, numhaulsBYsubdiv$SurvStratum), FUN=sum, na.rm=T))
  ...
  if(WRITE  & (!BOOTSTRAP | (BOOTSTRAP & B==0) ) ){
    ...
    if(BYSUBDIV){        
      if(!SAMP_STRAT){ 
        DATANOLEN<-aggregate(x=DATA$DensBiom_kg_Sqkm, by=list(Year=DATA$Year, SpeciesSciName=DATA$SpeciesSciName, Group=DATA$Group, HaulID=DATA$HaulID, SurvStratum=DATA$SurvStratum), FUN=sum)  
        species_rec_by_area <- aggregate(x=DATANOLEN$x, by=list(Year=DATANOLEN$Year, SpeciesSciName=DATANOLEN$SpeciesSciName, SurvStratum=DATANOLEN$SurvStratum), FUN=length)  
        names(species_rec_by_area)[length(names(species_rec_by_area))]<-"N"
        SPECIESSTRATRECS <- xtabs(N ~ Year + SpeciesSciName + SurvStratum,data=species_rec_by_area)
        for(i in 1:dim(SPECIESSTRATRECS)[3]){
          write.table( matrix(c( dimnames(SPECIESSTRATRECS)$SurvStratum[i], colnames(SPECIESSTRATRECS[,,i]) ),nrow=1),paste(FILENAM,SPECIES,"species_records_subdiv",i,"_yr.csv",sep="_"),row.names =F,col.names =F,sep=',',append = FALSE)
      ...
      if(SAMP_STRAT){ 
        DATANOLEN<-aggregate(x=DATA$DensBiom_kg_Sqkm, by=list(Year=DATA$Year, SpeciesSciName=DATA$SpeciesSciName, Group=DATA$Group, HaulID=DATA$HaulID, sampstrat=DATA$sampstrat,SurvStratum=DATA$SurvStratum), FUN=sum)  
        species_rec_by_area <- aggregate(x=DATANOLEN$x, by=list(Year=DATANOLEN$Year, SpeciesSciName=DATANOLEN$SpeciesSciName, sampstrat=DATANOLEN$sampstrat, SurvStratum=DATANOLEN$SurvStratum), FUN=length)  
        names(species_rec_by_area)[length(names(species_rec_by_area))]<-"N"
        SPECIESSTRATRECS <- xtabs(N ~ Year + SpeciesSciName + SurvStratum + sampstrat,data=species_rec_by_area)
      ...
      species_records_BY_reg<- aggregate(species_rec_by_area$N, by=list(Year=species_rec_by_area$Year, SpeciesSciName=species_rec_by_area$SpeciesSciName, 
                                                                        SurvStratum=species_rec_by_area$SurvStratum), FUN=sum)
      names(species_records_BY_reg)[length(names(species_records_BY_reg))]<-"N"
      species_records_BY_reg <- xtabs(N ~ Year + SpeciesSciName + SurvStratum,data=species_records_BY_reg)
  ...
  if(BYSUBDIV)  FACT <- c(FACT,"SurvStratum")
  ...
  # species_bio_by_area[species_bio_by_area$SurvStratum!=species_bio_by_area$SurvStratum,]
  # to average LD must add num hauls to bio data
  if(SAMP_STRAT){
    species_bio_by_area <- merge(x = species_bio_by_area,
                                 y = numhaulsBYsampstrat[,which(names(numhaulsBYsampstrat) != "STRAT_DIV" & names(numhaulsBYsampstrat) != "SurvStratum")], #avoid replicating names and creating .x .y
  ...
  #length(species_bio_by_area$DensBiom_kg_Sqkm[is.na(species_bio_by_area$DensBiom_kg_Sqkm)]) #check
  #length(species_bio_by_area$SurvStratum[is.na(species_bio_by_area$SurvStratum)]) #check
  ...
  # e.g. num rects sampled by SurvStratum  sumsampstrat_by_sub   ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  if(SAMP_STRAT){
    ...
    if(BYSUBDIV){ 
      numsampstrat_by_sub <- tapply.ID(df=numsampstrat_by_sea, datacols=c("numsampstrat"), factorcols=c("Year","SurvStratum"), sum,c("numsampstrat"))
      sumsampstrat_by_sub <- xtabs(numsampstrat ~ Year + SurvStratum, numsampstrat_by_sub)
    }
  ...
  if(BYSUBDIV){ #if no SAMP_STRAT and sampstrat=NA, then above gives same as this
    num_by_sub <- tapply.ID(df=species_bio_by_area, datacols=c("CatCatchWgtSwept"), 
                            factorcols=c("Year","SurvStratum"), sum,c("CatCatchWgtSwept")) 
    num_by_sub$numsamp <- 1
    sum_by_sub <- xtabs(numsamp ~ Year + SurvStratum, num_by_sub)
    ...
    #area sampled
    num_by_sub <- merge(x=num_by_sub,y=ATTRIB,by.x="SurvStratum",by.y="SurvStratum",all=T)
  ...
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  #LD+catch raised by kmsq for species    meanLD_bio_by_area[meanLD_bio_by_area$SurvStratum!=meanLD_bio_by_area$SurvStratum,]
  #summarise by sampstrat+SurvStratum raised by spatial area in kmsq and then kg->tonnes
  if(BYSUBDIV | SAMP_STRAT){
    ...
    if(SAMP_STRAT){ 
      ...
      meanLD_bio_by_area <- merge(x=ATTRIB, 
                                  y=species_bio_by_area[,!colnames(species_bio_by_area) %in% "SurvStratum"], 
                                  all.y=TRUE,
                                  by=c("sampstrat") ) 
    } else {
      meanLD_bio_by_area <- merge(x=ATTRIB, y=species_bio_by_area, all.y=TRUE, by.x=c("SurvStratum") , by.y=c("SurvStratum") ) 
      if( any(names(meanLD_bio_by_area) %in% "SurvStratum") & !any(names(meanLD_bio_by_area) %in% "SurvStratum") ) meanLD_bio_by_area$SurvStratum <-meanLD_bio_by_area$SurvStratum
      #meanLD_bio_by_area <- meanLD_bio_by_area[,-which(names(meanLD_bio_by_area) == "SurvStratum")] #rm duplicate col
    }
    #raise by area of lowest resolution of sampling strategy/SurvStratum
    if(AREASCALE){
    ...
    if(BYSUBDIV & !SAMP_STRAT & WRITE_LDs & (!BOOTSTRAP | (BOOTSTRAP & B==0)) ) write.csv(meanLD_bio_by_area,file = paste(paste(FILENAM,sep='_'),"LD_tonnes_Year_W.by.SurvStratum.csv",sep=".") )
    ...
    if(BYSUBDIV & SAMP_STRAT){
      ...
      #work out value to scale up SurvStratum by
      #lose species and length (otherwise inflate sum of areas)                           factorcols=c("sampstrat","Year","SurvStratum")
      ...
      #sum area by SurvStratum sampled
      #area_by_subdiv1 <- tapply.ID(df=area_by_subdiv, datacols=c("KM2_LAM"), factorcols=c("Year","SurvStratum"), sum,c("KM2_LAMsum"))  
      ...
      area_by_subdiv <- merge(x=ATTRIB, y=area_by_subdiv, all.y=TRUE, by.x=c("SurvStratum") , by.y=c("SurvStratum") ) 
      #ratio to scale up to SurvStratum estimate
      ...
      #now sum catch by sampstrat to SurvStratum area and scale for missing area (each year)
      FACT <- c("Year","FishLength_cm","SpeciesSciName","SurvStratum","DEMPEL","Order","Group")
      ...
      meanLD_bio_by_subdiv <- merge(x=area_by_subdiv[,c("SurvStratum","Year","scale")], y=meanLD_bio_by_subdiv, all.y=TRUE, by.x=c("SurvStratum","Year") , by.y=c("SurvStratum","Year") )  
      if(AREASCALE){
      ...
      if(WRITE_LDs & (!BOOTSTRAP | (BOOTSTRAP & B==0))) write.csv(meanLD_bio_by_subdiv,file = paste(paste(FILENAM,sep='_'), "LD_tonnes_Year_W.by.SurvStratum.csv",sep=".") )
    ...
    #third level biomass to regional sea (or survey extent i.e. coverage of 'SurvStratum' in year)
    for(SP in SPECIES){
  ...
  for(SP in SPECIES_LOOP){ # SP<-"ALL" 
    ...
    # include correction for area of strata here so have CPUE_estimates * area of sampstrat (or SurvStratum if lowest level)
    if(BYSUBDIV & SAMP_STRAT){ # merge in scaling factor
      species_bio_by_area <- merge(x=species_bio_by_area, y=area_by_subdiv, by.x = c("Year","SurvStratum"),by.y = c("Year","SurvStratum"),all.x=T)
      if(AREASCALE){
    ...
    }# corrected for any change in sampling between SurvStratum, but not scaled up to include missing SurvStratum
